...
- User settings: independent **beat** and **downbeat** MIDI pitch, timbre preset (or custom waveform), click duration, and Room + Echo (both map to one SoLoud echo filter). Stored in `MetronomeClickSoundSettings` (`lib/core/audio/metronome_click_sound_settings.dart`) via `SharedPreferences`. **Concert A4 (440 Hz) vs 432 Hz** is a separate general pref (`kGeneralConcertA4PrefsKey`, `referenceConcertPitchProvider` in `lib/features/settings/reference_concert_pitch_notifier.dart`); it drives equal-temperament `NoteMath.midiToHz` for click frequencies and for the tuner’s note/cents mapping. Legacy prefs (single base pitch + accent offset + old metronome-only A4 keys) migrate into the general key on load. After init, `AudioEngine.applyClickSoundSettings(settings, referenceA4Hz: …)` updates frequencies (`NoteMath.midiToHz`), waveform shape (`SoLoud.setWaveform`), and waveform scale (fixed at 1; SoLoud’s non–super-wave path does not apply scale to the main oscillator). `playMetronomeClick` awaits `play()` with **per-instance volume** (0.35 beat / 0.55 downbeat), applies per-voice echo wet/delay/decay via `metronomeEchoParamsFromSliders` (`lib/core/audio/metronome_echo_mapping.dart`) when not on Web, and `scheduleStop` truncates the click to the chosen duration (downbeat uses a slightly longer gate). Mono sources use the echo filter for room/slapback; freeverb is not used because it requires stereo sources.
...
The gear action opens a **full-screen** `SettingsPage` (`lib/features/settings/settings_page.dart`) pushed on the root navigator, with **General** (accent color, **concert pitch** 440/432 Hz, tuner pitch-strip edge, and the privacy line). The layout scales to fit the viewport without scrolling. `openSettings` and `AppSettingsButton` live in that file; accent presets and `AccentColorSection` remain in `accent_settings.dart`; concert pitch uses `ConcertPitchSection` / `referenceConcertPitchProvider`.

Metronome click synthesis, dual pitch rows (Hz readouts follow the global reference), effects, shape preview, and audio preview use `MetronomeClickSoundSettings` / `MetronomeClickSoundSection` on a **separate full-screen** `MetronomeSoundPage` (`lib/features/metronome/metronome_sound_page.dart`, scrollable) opened from **Metronome sound** on the Metronome tab (below transport, not from the gear menu). The main Metronome and Tuner tabs use viewport-filling `Column`/`Row` layouts with `Expanded`/`Flexible` (no scroll); **Settings** uses a `ConstrainedBox` without letterboxing the whole page.
...
- `NoteMath` maps Hz to the nearest chromatic label and **cents** in **−50…+50** relative to that note (equal temperament). The reference A4 frequency is **440 Hz** (concert) or **432 Hz** from **General** settings; `PitchDetector.analyzeFrame` receives that value per frame via `TunerAudioController`.
